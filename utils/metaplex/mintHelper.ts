...
export const mintArgsBuilder = (
  ...
  for (let i = 0; i < amount; i++) {
    ...
    if (guards.allowList.__option === "Some") {
      if (!_top10Wallets.length) {
        throw new Error(
          `Allowlist guard is active but _top10Wallets is empty. Did you forget to call cacheLeaderboard()?`
        );
      }

      const allowlist = _top10Wallets; // already sorted in cacheLeaderboard
      const merkleRoot = getMerkleRoot(allowlist);
      args.allowList = some({ merkleRoot });
    }
...
export async function sendAllowListProof(
  ...
  if (!_top10Wallets.length) {
    throw new Error(
      `Allowlist guard is active but _top10Wallets is empty in sendAllowListProof.`
    );
  }

  const allowlist = _top10Wallets; // sorted
  const merkleRoot = getMerkleRoot(allowlist);
